...
class NonPersonnalizedRecommenderSuite extends FunSuite {
  test("simple with movie 11") {
    val actual = NonPersonnalizedRecommender.recommender(
        getClass().getResource("recsys-data-movie-titles.csv").getPath(), 
        getClass().getResource("recsys-data-users.csv").getPath(), 
        getClass().getResource("recsys-data-ratings.csv").getPath(), 
        11, true)
